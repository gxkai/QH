...
<script>
import NOH from '@/components/goods/goods/NOH'
import Type from '@/components/goods/goods/Type'
  ...
  data() {
    return {
      ...
      addRules: {
        ...
        NOH: [{ required: true, message: '请输入名称', trigger: 'blur' }],
        unit: [{ required: true, message: '请输入名称', trigger: 'blur' }],
  ...
  methods: {
    ...
    async handleRowEdit({ index, row }, done) {
      ...
      try {
        ...
        await this.$axios.put(`/api/v1/goods/${row._id}`, {
          name: row.name,
          code: row.code,
          type: row.type,
          unit: row.unit,
    ...
    async handleRowRemove({ index, row }, done) {
      ...
      try {
        this.formOptions.saveLoading = true
        await this.$axios.$delete(`/api/v1/goods/${row._id}`)
        this.$message({
          message: '删除成功',
          type: 'success'
        })
    ...
    async handleRowAdd(row, done) {
      ...
      try {
        this.formOptions.saveLoading = true
        await this.$axios.post(`/api/v1/goods`, row)
        this.$message({
          message: '保存成功',
          type: 'success'
        })
    ...
    handleSpecAdd({ row, index }) {
      this.$router.push({
        path: '/goods/goods/goodsSpec',
        query: { goods: row._id }
      })
    }
  }
