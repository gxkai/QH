<template>
  <div class="mt-5">
    ...
    <el-row class="mb-5 w-1/6 mt-5" slot="header">
  ...
  methods: {
    ...
    async handleRowEdit({ index, row }, done) {
      ...
      try {
        ...
        await this.$axios.put(`/api/v1/company/${row._id}`, { name: row.name })
        this.$message({
          message: '编辑成功',
          type: 'success'
        })
    ...
    async handleRowRemove({ index, row }, done) {
      ...
      try {
        this.formOptions.saveLoading = true
        await this.$axios.$delete(`/api/v1/company/${row._id}`)
        this.$message({
          message: '删除成功',
          type: 'success'
        })
    ...
    async handleRowAdd(row, done) {
      ...
      try {
        this.formOptions.saveLoading = true
        await this.$axios.post(`/api/v1/company`, row)
        this.$message({
          message: '保存成功',
          type: 'success'
        })
