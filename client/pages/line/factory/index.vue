<template>
  <div class="mt-5">
    <el-button slot="header" @click="addRowWithNewTemplate">{{
      $t(`button.add`)
    }}</el-button>
  ...
  methods: {
    ...
    async handleRowEdit({ index, row }, done) {
      ...
      try {
        ...
        await this.$axios.put(`/api/v1/factory/${row._id}`, {
          name: row.name,
          company: row.company
        })
        this.$message({
    ...
    async handleRowRemove({ index, row }, done) {
      ...
      try {
        this.formOptions.saveLoading = true
        await this.$axios.$delete(`/api/v1/factory/${row._id}`)
        this.$message({
          message: '删除成功',
          type: 'success'
        })
    ...
    async handleRowAdd(row, done) {
      ...
      try {
        this.formOptions.saveLoading = true
        await this.$axios.post(`/api/v1/factory`, row)
        this.$message({
          message: '保存成功',
          type: 'success'
        })
